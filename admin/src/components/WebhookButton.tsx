...
export default function WebhookButton(props: ButtonConfig) {
	const data = useContentManagerContext() as ContentManagerData & {
		form: { values: Record<string, any> };
	};
	const [loading, setLoading] = useState(false);
	const [alert, setAlert] = useState<{ variant: 'success' | 'danger'; message: string } | null>(
		null
	);
			...
			{alert && (
				<Portal>
					<Flex
						...
						alignItems="center"
						style={{ position: 'absolute', top: 0, left: 0, zIndex: 700, width: '100%' }}
					>
						<Alert
